...
import { getGameById, isPinedGame, isActiveGame, getActualRound } from '@state/getters'
...
const Game = (props) => {
  ...
  if (!isEmpty(props.search) && !startsWith(game.name, toUpper(props.search))) return null

  const profitPercents = futureProfitPercent(actualround.prizefunds, game.minWager)
  const [maxProfitPosition, maxProfitPercents] = reduce(profitPercents, (acc, percents, position) => {
            ...
            <div className={css.top}>
              ...
              {maxProfitPosition && maxProfitPercents && (
                <div className={css.profit}>
                  {Number(maxProfitPosition) === PRIZEFUNDS.UP && <BidUpIcon/>}
                  {Number(maxProfitPosition) === PRIZEFUNDS.DOWN && <BidDownIcon/>}
                  {Number(maxProfitPosition) === PRIZEFUNDS.EQUAL && <BidZeroIcon/>}
                  <div className={css.percent}>{htmlPercentSigned(Math.max(maxProfitPercents, 0))}</div>
                </div>
              )}
            </div>
