...
import css from '@styles/components/roundprizefundsbar/index.module.scss'
...
const RoundPrizefundsBar = (props) => {
  ...
  return (
    <div className={cn(css.roundprizefundsbar, { [css.idel]: prizefund === 0 })}>

      {!!prizefundZeroPercent &&
        <div className={cn(css.percent, css.perzero)}
          style={{ top: htmlPercent(prizefundUpPercent + prizefundZeroPercent/2) }}
        >
      ...
      {!!prizefundUpPercent &&
        <div className={cn(css.percent, css.perup)}>{htmlPercent(prizefundUpPercent)}</div>
      }

      <div className={css.lines}>
        <div className={css.lineup} style={{ height: htmlPercent(prizefundUpPercent) }}></div>
        <div className={css.linezero} style={{ height: htmlPercent(prizefundZeroPercent) }}></div>
        <div className={css.linedown} style={{ height: htmlPercent(prizefundDownPercent) }}></div>
      </div>

      {!!prizefundDownPercent &&
        <div className={cn(css.percent, css.perdown)}>{htmlPercent(prizefundDownPercent)}</div>
      }
