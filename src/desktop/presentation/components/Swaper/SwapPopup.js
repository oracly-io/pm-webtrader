...
const SwapPopup = (props) => {
  ...
  }, [erc20, props.close, props.isCompletedSwap])
  ...
  const swap = useCallback(() => {
    ...
    props.SWAP_NATIVE_ERC20({ account, erc20, swapid: activeSwap.swapid,  quote: activeQuote, pooldata: activePool })

  }, [account, activeQuote, activePool])

  const changeAmountIn = useCallback((amountIn) => {
    props.UPDATE_SWAP({ erc20, update: { amountIn, amountOut: null } })
  }, [erc20])

  const changeAmountOut = useCallback((amountOut) => {
  ...
  const insufficientLowGas = gt(activeQuote?.amountIn, sub(props.gas, config.low_gas))

  const exRates = div(activeQuote?.amountIn, activeQuote?.amountOut)
  const insufficientNative = gt(mul(activeSwap.minAmountOut, exRates), sub(props.gas, config.low_gas))
  const addNative = sub(mul(activeSwap.minAmountOut, exRates), sub(props.gas, config.low_gas))
    ...
    <div className={css.container}>
      <div className={css.header}>
        <span className={css.title}>
          {t('Add {{currency}}', { currency: ERC20[erc20] })}
        </span>
      ...
      <div className={css.swaper}>
        {!insufficientLowGas && insufficientSwap &&
          <div className={css.error}>
            ...
            <div className={css.message}>
              {t('A minimum swap of {{amount}} {{currency}} is required to support desired play.', {
        ...
        {insufficientLowGas && !insufficientSwap &&
          <div className={css.error}>
            <ErrorIcon />
            <div className={css.message}>
              {t('At least {{amount}} {{currency}} must remain on the balance to play.', {
        ...
        {insufficientNative && (
          ...
          <div className={css.error}>
            <ErrorIcon />
            <div className={css.message}>
              {t('At least {{amount}} {{currency}} must be added to the balance to support desired play.', {
        ...
        <div className={cn(css.amountIn, { [css.errored]: insufficientLowGas, propagating: props.isPendingTxn })}>

          <div className={css.amountSell}>
            <div className={css.amountTitle}>{t('Sell')}</div>
            <div className={cn(css.amount, { [css.disabled]: !activeQuote?.amountIn })}>
              ...
              {insufficientLowGas &&
                <div className={css.info}
                  title={t(
                    ...
                    { amount: htmlCurrency(config.low_gas), currency: NATIVE.SYMBOL }
                  )}
                >
                  <ErrorIcon/>
                </div>
        ...
        <div className={cn(css.amountOut, { [css.errored]: insufficientSwap, propagating: props.isPendingTxn })}>
              ...
              {insufficientSwap &&
                <div className={css.info}
                  title={t(
                    ...
                    { amount: htmlCurrency(activeSwap.minAmountOut), currency: ERC20[erc20] }
                  )}
                >
                  <ErrorIcon/>
                </div>
              }
              {t('Balance: {{balance}}', { balance: htmlCurrency(props.balance) })}
            </div>
          </div>
      ...
      <ul className={css.list}>
        <li className={css.item}>
          <div className={css.exrate}>
            {t('1 {{OUT}} = {{rate}} {{IN}}', { OUT: ERC20[erc20], IN: NATIVE.SYMBOL, rate: htmlCurrency(exRates) })}
          </div>
        </li>
        <li className={css.item}>
...
export default connect(
  (state) => {
    ...
    return {
      ...
      isPendingTxn: !isEmpty(getPendingTransactionsByEntityId(state, hexHash({ SWAP_NATIVE_ERC20, swapid: activeSwap.swapid }))),
      isPendingAct: isInProgress(getAsyncStatus(state, SWAP_NATIVE_ERC20, [activeSwap.swapid])),

      isCompletedSwap: !isEmpty(getComittedTransactionsByEntityId(state, hexHash({ SWAP_NATIVE_ERC20, swapid: activeSwap.swapid }))),
    }
  },
  ({ command, query }) => ([
    command(END_SWAP),
