...
import { formatDistanceUnixTS, formatUnixTS, nowUnixTS, toLocalStringUnixTS } from '@oracly/pm-libs/date-utils'
import { htmlPercentSigned, htmlCurrency, htmlCurrencyNamed } from '@oracly/pm-libs/html-utils'

import { SET_ACTIVE_GAME, FOCUS_ROUND } from '@actions'
import { READ_BLOCKCHAIN_ROUND_BY_ID } from '@actions'

import { DOWN, UP, EQUAL, NOCONTEST, UNDEFINED, PRICEFEED } from '@constants'
...
import { getGameById, getPredictionById, getRoundByPredictionId } from '@state/getters'
import { getSettlmentById, getLatestPriceByPricefeed } from '@state/getters'
import { isEntityPropagating, isEnityVerified, isEnityReverted } from '@state/getters'
...
const Prediction = (props) => {
  ...
  useEffect(() => {
    if (isEmpty(round) && !isEmpty(prediction) && blockNumber) {
      props.READ_BLOCKCHAIN_ROUND_BY_ID({ roundid: prediction.roundid, txn: { blockNumber } })
    }
      ...
      <div className={css.bottom}>
        ...
        <div className={css.info}>
          <div className={css.wager}>
            ...
            <LevelCurrency className={css.currency} currency={game.currency} type={game.level} />
          </div>
