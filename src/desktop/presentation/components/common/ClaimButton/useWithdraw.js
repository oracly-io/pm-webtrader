...
export const useWithdraw = (props, {
  ...
  if (!(props.WITHDRAW && props.RESOLVE_WITHDRAW_NOCONTEST && props.RESOLVE_WITHDRAW)) {
    console.error(`It seems you forgot to connect some of action/actions: ${WITHDRAW}, ${RESOLVE_WITHDRAW}, ${RESOLVE_WITHDRAW_NOCONTEST}`) // eslint-disable-line
  }
  ...
  const withdraw = useCallback((e) => {
    ...
    if (!round?.resolved && !isEmpty(round?.roundid)) {
      ...
      if (emptyround || expired) {
        ...
        props.RESOLVE_WITHDRAW_NOCONTEST && props.RESOLVE_WITHDRAW_NOCONTEST({
          roundid: round.roundid,
          predictionid: prediction.predictionid,
          erc20: prediction.erc20,
        })
        ...
        if (!isEmpty(settlment)) {
          ...
          props.RESOLVE_WITHDRAW && props.RESOLVE_WITHDRAW({
            roundid: round.roundid,
            predictionid: prediction.predictionid,
            erc20: prediction.erc20,
