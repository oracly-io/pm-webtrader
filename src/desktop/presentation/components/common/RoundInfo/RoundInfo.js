...
import { getActiveAccountAddress, getBettorRoundPredictions, getGameById } from '@state/getters'
...
const RoundInfo = (props) => {
  ...
  useEffectState((state) => {
    ...
    if (blockNumber && round?.roundid) {
       props.READ_BLOCKCHAIN_ROUND_BY_ID({ roundid: round.roundid, txn: { blockNumber } })
    }
  }, [round?.roundid])
  ...
  useScheduledQuery((query, state) => {
    ...
    if (blockNumber && round?.roundid && round?.phantom) {
      query(READ_BLOCKCHAIN_ROUND_BY_ID, { roundid: round.roundid, txn: { blockNumber } }, { schedule: 5 })
    }
  }, [round?.roundid])
  ...
  return (
    <div ref={ref} className={cn(css.roundinfo, props.className, {
      propagating,
      phantom,
    })}>

      {props.children({ round, settlment, game, resolution, propagating, bettorpredictions }, ref)}
...
export default connect(
  (state, { round }) => {
    const bettorpredictions = getBettorRoundPredictions(state, round.roundid, getActiveAccountAddress(state))
