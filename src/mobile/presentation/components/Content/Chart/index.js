...
import {
  getActiveGameBettorPredictions,
  getActiveGameSettlmentsByIds,
} from '@state/getters'
import {
  getActiveGame,
  pickAsyncStatus,
...
export const Chart = (props) => {
  ...
  useEffect(() => {
    if (!ready && succeed && chart && props.game?.schedule) {
      chart.setTimeframe(props.game?.schedule * 0.4)
      setReady(true)
    }
  }, [succeed, chart, props.game?.schedule])

  useEffect(() => {
    if (chart && props.game?.schedule) {
      chart.setTimeframe(props.game?.schedule * 0.4)
    }
  }, [chart, props.game?.schedule])
  ...
  useEffect(
    (state) => {
      const firstReq = isNeverPerformed(
        props,
        READ_BLOCKCHAIN_LATEST_ROUNDS_BY_GAMEID
      )
      if (gameid && blockNumber && firstReq) {
        props.READ_BLOCKCHAIN_LATEST_ROUNDS_BY_GAMEID({
          gameid,
          txn: { blockNumber },
        })
      }
  ...
  const setChartSize = () => {
    ...
    chart.setScreenSize({ width, height: height - HEADER_HEIGHT })
  }
  ...
  useEffect(() => {
    ...
    screen.orientation.addEventListener('change', setChartSize)
    ...
    chart.addEventListener('withdraw', (e) => {
    ...
    chart.addEventListener('resolvewithdraw', (e) => {
      if (
        e.roundid &&
        e.predictionid &&
        e.erc20 &&
        e.exitPrice?.roundid &&
        e.controlPrice?.roundid
      ) {
    ...
    chart.addEventListener('resolvewithdrawnocontest', (e) => {
    ...
    chart.addEventListener('roundpin', (e) => {
      if (e.roundid) {
        props.FOCUS_ROUND({ roundid: e.roundid })
      }
    })
    chart.addEventListener('roundunpin', (e) => {
      if (e.roundid) {
        props.UNFOCUS_ROUND({ roundid: e.roundid })
      }
    })
    chart.addEventListener('timeframeTonow', (e) => {
      props.TIMEFRAME_NOW_STICK()
    })
    chart.addEventListener('timeframeUnnow', (e) => {
...
export default connect(
  (state) => {
    ...
    const predictions = groupBy(
      getActiveGameBettorPredictions(state, bettor),
      'roundid'
    )
    const chartdata = getActiveGameChartData(state)
    const settlments = getActiveGameSettlmentsByIds(
      state,
      map(rounds, 'endDate')
    )
    ...
    return {
      ...
      ...pickAsyncStatus(state, READ_BLOCKCHAIN_LATEST_ROUNDS_BY_GAMEID, [
        game?.gameid,
      ]),
    }
  },
