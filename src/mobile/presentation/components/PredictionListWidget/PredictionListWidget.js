...
import { TOOLBAR_PREDICTIONS } from '@constants'
...
import {
  getPredictionsTotalSizeByAddress,
  getBettorPredictionIds,
  isScrollFailed,
  isScrollLoading,
} from '@state/getters'
import {
  getPredictionsByIds,
  getPredictionsLoadedSizeByAddress,
  getRoundsByIds,
} from '@state/getters'
import {
  getPredictionsListOffset,
  getLatestbcBlockNumber,
} from '@state/getters'
import {
  getPredictionsMissingRoundIds,
  getMissingSettlments,
} from '@state/getters'
...
const VirtuosoList = forwardRef((props, ref) => (
  <div className={css.list} {...props} ref={ref} />
))
VirtuosoList.displayName = 'VirtuosoList'

const VirtuosoItem = ({ item, ...props }) => (
  <div className={predictioncss.container} {...props} />
)

const VirtuosoFooter = ({
  context: { loadMore, hasMore, isFailed, isLoading },
}) => {
...
const virtuoso = {
  ...
  itemContent: (index, prediction) => (
    <Prediction predictionid={prediction.predictionid} />
  ),
}
...
const PredictionListWidget = (props) => {
  ...
  const atBottomStateChangeHandler = useCallback(
    (atBottom) => {
  ...
  return (
    <div className={css.predictionlistwidget}>
      <div className={css.header}>
        ...
        <Button
          className={css.close}
          onClick={() => props.TOGGLE_TOOLBAR({ type: TOOLBAR_PREDICTIONS })}
        >
          <CloseIcon />
        </Button>
      </div>
...
export default connect(
  (state) => {
    ...
    const loadPredictionsFailed = isScrollFailed(
      state,
      READ_BLOCKCHAIN_LATEST_PREDICTIONS_BY_ADDRESS,
      [bettorid]
    )
    const loadPredictionsInProgress = isScrollLoading(
      state,
      READ_BLOCKCHAIN_LATEST_PREDICTIONS_BY_ADDRESS,
      [bettorid]
    )
