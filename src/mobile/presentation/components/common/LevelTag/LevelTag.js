...
const LevelTag = ({ className, game, short }) => {
  ...
  let level = short ? t('B') : t('Bronze')
  if (isSilver) level = short ? t('S') : t('Silver')
  if (isGold) level = short ? t('G') : t('Gold')

  return (
    <div
      className={cn(css.leveltag, className, {
        [css.bronze]: isBronze,
        [css.silver]: isSilver,
        [css.gold]: isGold,
      })}
      title={t(
        'Round Level "{{level}}" minimum wager is {{minWager}} {{currency}}',
